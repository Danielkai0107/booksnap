...
export function issueReportCategoryLabel(value: IssueReportCategory): string {
  return ISSUE_REPORT_CATEGORIES.find((c) => c.value === value)?.label ?? value;
}
...
const STATUS_SET = new Set<string>(ISSUE_REPORT_STATUSES.map((s) => s.value));
